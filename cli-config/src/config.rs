...
lazy_static! {
    ...
    pub static ref CONFIG_FILE: Option<String> = {
        dirs_next::home_dir().map(|mut path| {
            path.extend([".config", "dolly", "cli", "config.yml"]);
...
pub struct Config {
    ...
    ///
    /// [rpcdocs]: https://dolly.com/docs/core/clusters
    ...
    /// documentation for `dolly_clap_utils::keypair::signer_from_path`.
    ...
    /// function, or with `dolly_clap_utils::keypair::DefaultSigner`.
    ...
    /// `dolly_sdk::commitment_config::CommitmentLevel::Confirmed`.
    #[serde(default)]
...
impl Default for Config {
    fn default() -> Self {
        let keypair_path = {
            ...
            keypair_path.extend([".config", "dolly", "id.json"]);
            keypair_path.to_str().unwrap().to_string()
        };
        let json_rpc_url = "https://api.mainnet-beta.dolly.com".to_string();
...
mod test {
    ...
    fn compute_websocket_url() {
        assert_eq!(
            Config::compute_websocket_url("http://api.devnet.dolly.com"),
            "ws://api.devnet.dolly.com/".to_string()
        );

        assert_eq!(
            Config::compute_websocket_url("https://api.devnet.dolly.com"),
            "wss://api.devnet.dolly.com/".to_string()
        );
