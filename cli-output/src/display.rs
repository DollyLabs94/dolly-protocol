...
fn write_instruction<'a, W: io::Write>(
    ...
    if let AccountKeyType::Known(program_pubkey) = program_pubkey {
        if program_pubkey == &dolly_vote_program::id() {
            if let Ok(vote_instruction) = limited_deserialize::<
                dolly_vote_program::vote_instruction::VoteInstruction,
            >(&instruction.data)
            {
        ...
        } else if program_pubkey == &dolly_sdk::system_program::id() {
            if let Ok(system_instruction) = limited_deserialize::<
                dolly_sdk::system_instruction::SystemInstruction,
            >(&instruction.data)
            {
...
mod test {
    use {
        ...
        dolly_transaction_status::{Reward, RewardType, TransactionStatusMeta},
        std::io::BufWriter,
    };
