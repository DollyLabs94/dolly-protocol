...
//!
//! This crate implements 3 dolly-runtime traits (`InstalledScheduler`, `UninstalledScheduler` and
...
//! `dolly-ledger`'s helper function called `execute_batch()`.
...
use {
    ...
    log::*,
    dolly_ledger::blockstore_processor::{
    ...
    dolly_sdk::transaction::{Result, SanitizedTransaction},
    dolly_unified_scheduler_logic::Task,
...
// SchedulerPool must be accessed as a dyn trait from dolly-runtime, because SchedulerPool
// contains some internal fields, whose types aren't available in dolly-runtime (currently
...
mod tests {
    ...
    fn test_scheduler_schedule_execution_failure() {
        ...
        assert_matches!(
            ...
            Some((
                Err(dolly_sdk::transaction::TransactionError::AccountNotFound),
                _timings
            ))
        );
    }
