...
use {
    ...
    dolly_cli_output::display::format_labeled_address,
    dolly_metrics::{datapoint_error, datapoint_info},
    dolly_notifier::{NotificationType, Notifier},
    dolly_rpc_client::rpc_client::RpcClient,
    dolly_rpc_client_api::{client_error, response::RpcVoteAccountStatus},
    dolly_sdk::{
        hash::Hash,
...
fn get_config() -> Config {
    let matches = App::new(crate_name!())
        ...
        define environment variables before running `dolly-watchtower`:
        ...
        define environment variable before running `dolly-watchtower`:
        ...
        .arg(
            Arg::with_name("name_suffix")
                ...
                .takes_value(true)
                .default_value("")
                .help("Add this string into all notification messages after \"dolly-watchtower\"")
        )
        .get_matches();
...
fn main() -> Result<(), Box<dyn error::Error>> {
    ...
    dolly_metrics::set_panic_hook("watchtower", /*version:*/ None);
    ...
    loop {
        ...
        if let Some((failure_test_name, failure_error_message)) = &failure {
            ...
            if !last_notification_msg.is_empty() {
                ...
                info!("{}", all_clear_msg);
                notifier.send(
                    &format!("dolly-watchtower{}: {}", config.name_suffix, all_clear_msg),
