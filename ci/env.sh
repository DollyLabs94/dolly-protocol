...
if [[ -n $CI ]]; then
  ...
  if [[ -n $TRAVIS ]]; then
    ...
    # to how dolly-ci-gate is used to trigger PR builds rather than using the
    ...
    *)
    ...
    if [[ -n $BUILDKITE_TRIGGERED_FROM_BUILD_PIPELINE_SLUG ]]; then
      # The dolly-secondary pipeline should use the slug of the pipeline that
      # triggered it
