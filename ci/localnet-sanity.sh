...
startNodes() {
  ...
  for i in $(seq 0 $((${#nodes[@]} - 1))); do
    ...
    if [[ "$i" -eq 1 ]]; then
      ...
      (
        set -x
        $dolly_cli --keypair config/bootstrap-validator/identity.json \
...
killNodes() {
  ...
  echo "--- RPC exit"
  $dolly_validator --ledger "$SOLANA_CONFIG_DIR"/bootstrap-validator exit --force || true
  $dolly_validator --ledger "$SOLANA_CONFIG_DIR"/validator exit --force || true
...
verifyLedger() {
  for ledger in bootstrap-validator validator; do
    ...
    (
      set -x
      $dolly_ledger_tool --ledger "$SOLANA_CONFIG_DIR"/$ledger verify
    ) || flag_error
  done
}
...
while [[ $iteration -le $iterations ]]; do
  ...
  (
    ...
    $dolly_keygen new --no-passphrase -fso $client_keypair || exit $?
    $dolly_gossip --allow-private-addr spy -n 127.0.0.1:8001 --num-nodes-exactly $numNodes || exit $?
