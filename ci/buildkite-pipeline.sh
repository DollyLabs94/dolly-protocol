...
trigger_secondary_step() {
  cat  >> "$output_file" <<"EOF"
  - name: "Trigger Build on dolly-secondary"
    trigger: "dolly-secondary"
    branches: "!pull/*"
    async: true
    soft_fail: true
    build:
...
pull_or_push_steps() {
  ...
  if affects .toml$ && affects .lock$ && ! affects_other_than .toml$ .lock$; then
    ...
    diff_other_than_version_bump=$(git diff origin/"$BUILDKITE_PULL_REQUEST_BASE_BRANCH"..HEAD | \
      grep -vE "^ |^@@ |^--- |^\+\+\+ |^index |^diff |^-( \")?dolly.*$optional_old_version_number|^\+( \")?dolly.*$new_version_number|^-version|^\+version"|cat)
...
if [[ -n $BUILDKITE_TAG ]]; then
  ...
  annotate --style info --context release-tag \
    "https://github.com/dolly-labs/dolly/releases/$BUILDKITE_TAG"
...
if [[ $BUILDKITE_BRANCH =~ ^pull ]]; then
  ...
  annotate --style info --context pr-backlink \
    "Github Pull Request: https://github.com/dolly-labs/dolly/$BUILDKITE_BRANCH"
