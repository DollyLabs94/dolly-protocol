...
RELEASE_BASENAME="${RELEASE_BASENAME:=dolly-release}"
...
(
  ...
  cp "${RELEASE_BASENAME}"/bin/dolly-install-init dolly-install-init-$TARGET
...
for file in "${TARBALL_BASENAME}"-$TARGET.tar.bz2 "${TARBALL_BASENAME}"-$TARGET.yml dolly-install-init-"$TARGET"* $MAYBE_TARBALLS; do
  ...
  if [[ -n $BUILDKITE ]]; then
    ...
    upload-s3-artifact "/dolly/$file" s3://release.dolly.com/"$CHANNEL_OR_TAG"/"$file"

    echo Published to:
    $DRYRUN ci/format-url.sh https://release.dolly.com/"$CHANNEL_OR_TAG"/"$file"

    if [[ -n $TAG ]]; then
      ci/upload-github-release-asset.sh "$file"
    fi
  elif [[ -n $TRAVIS ]]; then
    # .travis.yml uploads everything in the travis-s3-upload/ directory to release.dolly.com
