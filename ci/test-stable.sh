...
case $testName in
test-stable)
  _ ci/intercept.sh cargo test --jobs "$JOBS" --all --tests --exclude dolly-local-cluster ${V:+--verbose} -- --nocapture
  ;;
test-stable-sbf)
  ...
  # dolly-keygen required when building C programs
  ...
  # SBF dolly-sdk legacy compile test
  ...
  # expected number of dolly-program crate compilations is 4. There
  # should be 3 builds of dolly-program while 128bit crate is
  ...
  # dolly-program is performed when simulation crate is built. This
  # last compiled dolly-program is of different version, normally the
  # latest mainbeta release version.
  dolly_program_count=$(grep -c 'dolly-program v' cargo.log)
  rm -f cargo.log
  if ((dolly_program_count > 20)); then
      echo "Regression of build redundancy ${dolly_program_count}."
      echo "Review dependency features that trigger redundant rebuilds of dolly-program."
      exit 1
  fi
  ...
  tar cjvf "$sbf_dump_archive" "${sbf_target_path}"/{deploy/*.txt,sbf-dolly-dolly/release/*.so}
  exit 0
  ;;
test-stable-perf)
  if [[ $(uname) = Linux ]]; then
    ...
    # Force CUDA for dolly-core unit tests
  ...
  _ cargo test --package dolly-perf --package dolly-ledger --package dolly-core --lib ${V:+--verbose} -- --nocapture
...
test-wasm)
  ...
  ;;
test-docs)
  _ cargo test --jobs "$JOBS" --all --doc --exclude dolly-local-cluster ${V:+--verbose} -- --nocapture
  exit 0
  ;;
*)
...
(
  ...
  export RUST_LOG="dolly_metrics=warn,info,$RUST_LOG"
