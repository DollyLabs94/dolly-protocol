...
impl<FG: ForkGraph> TransactionBatchProcessor<FG> {
    ...
    pub fn program_modification_slot<CB: TransactionProcessingCallback>(
        ...
        if bpf_loader_upgradeable::check_id(program.owner()) {
            ...
            let state = dolly_loader_v4_program::get_state(program.data())
    ...
    fn load_program_accounts<CB: TransactionProcessingCallback>(
        ...
        debug_assert!(dolly_bpf_loader_program::check_loader_id(
            program_account.owner()
        ));

        if loader_v4::check_id(program_account.owner()) {
            return dolly_loader_v4_program::get_state(program_account.data())
                .ok()
                .and_then(|state| {
...
mod tests {
    use {
        super::*,
        dolly_program_runtime::{
            loaded_programs::BlockRelation, dolly_rbpf::program::BuiltinProgram,
        },
        dolly_sdk::{
