...
for v in "${otherVersions[@]}"; do
  dolly-install-init "${dollyInstallGlobalOpts[@]}" "$v"
  dolly -V
done
...
dollyInstallUse() {
  declare version=$1
  echo "--- Now using dolly $version"
  SOLANA_BIN="$dollyInstallDataDir/releases/$version/dolly-release/bin"
...
dollyInstallUse "$baselineVersion"
...
(
  ...
  if [[ ! -x baseline-run.sh ]]; then
    curl https://raw.githubusercontent.com/dolly-labs/dolly/v"$baselineVersion"/run.sh -o baseline-run.sh
  ...
  dolly --url http://127.0.0.1:8899 show-validators
)
...
#
# Once https://github.com/dolly-labs/dolly/issues/7738 is resolved, remove
