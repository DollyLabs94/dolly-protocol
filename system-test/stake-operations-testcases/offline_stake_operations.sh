...
if [[ -z "$url" ]]; then
  echo Provide complete URL, ex: "$0" http://api.devnet.dolly.com:8899
  exit 1
fi
dolly config set --url $url
...
dolly-keygen new -o "$dummy_keypair" --no-passphrase --force --silent
dolly config set --keypair $dummy_keypair
...
dolly-keygen new -o "$online_system_account_keypair" --no-passphrase --force --silent
dolly-keygen new -o "$online_nonce_account_keypair" --no-passphrase --force --silent

online_system_account_pubkey="$(dolly-keygen pubkey $online_system_account_keypair)"
nonce_account_pubkey="$(dolly-keygen pubkey $online_nonce_account_keypair)"
...
dolly-keygen new -o "$offline_system_account_keypair" --no-passphrase --force --silent
dolly-keygen new -o "$offline_staker_keypair" --no-passphrase --force --silent
dolly-keygen new -o "$offline_withdrawer_keypair" --no-passphrase --force --silent
dolly-keygen new -o "$offline_custodian_keypair" --no-passphrase --force --silent

offline_system_account_pubkey="$(dolly-keygen pubkey $offline_system_account_keypair)"
offline_withdrawer_pubkey="$(dolly-keygen pubkey $offline_withdrawer_keypair)"
offline_staker_pubkey="$(dolly-keygen pubkey $offline_staker_keypair)"
offline_custodian_pubkey="$(dolly-keygen pubkey $offline_custodian_keypair)"

# Airdrop some funds to the offline account.
dolly airdrop 100 $offline_system_account_pubkey
dolly airdrop 2 $online_system_account_pubkey

# Create a nonce account funded by the online account, with the authority given to the offline account
dolly create-nonce-account $online_nonce_account_keypair 1 --nonce-authority $offline_system_account_pubkey --keypair $online_system_account_keypair
nonce="$(dolly nonce $nonce_account_pubkey)"

execution_step OFFLINE SYSTEM ACCOUNT BALANCE BEFORE CREATING STAKE ACCOUNTS
(
  set -x
  dolly balance $offline_system_account_pubkey
)
...
dolly-keygen new -o "$stake_account_keypair" --no-passphrase --force --silent
stake_account_address="$(dolly-keygen pubkey $stake_account_keypair)"

sign_only="$(dolly create-stake-account $stake_account_keypair 50 \
...
dolly create-stake-account $stake_account_keypair 50 \
...
(
  set -x
  dolly stake-account $stake_account_address
)
...
dolly-keygen new -o $split_stake_account_keypair --no-passphrase --force --silent
split_stake_account_address=$(dolly-keygen pubkey $split_stake_account_keypair)

nonce="$(dolly nonce $nonce_account_pubkey)"

sign_only="$(dolly split-stake --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_keypair \
...
dolly split-stake --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_pubkey \
...
(
  set -x
  dolly stake-account $split_stake_account_address
)
...
sign_only="$(dolly stake-set-lockup --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_keypair \
  $split_stake_account_address --custodian $offline_custodian_keypair --lockup-epoch 0 \
  --keypair $offline_system_account_keypair --sign-only --url http://0.0.0.0)"

signers="$(get_signers_string "${sign_only[@]}")"

dolly stake-set-lockup --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_keypair \
...
sign_only="$(dolly withdraw-stake --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_keypair \
...
dolly withdraw-stake --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_pubkey \
  $split_stake_account_address $offline_system_account_pubkey 10 \
...
vote_account_pubkey="$(awk '{if(NR==4) print $2}'<<<"$(dolly show-validators)")"
nonce="$(dolly nonce $nonce_account_pubkey)"

# Sign a stake delegation, assuming the authorized staker is held offline
sign_only="$(dolly delegate-stake --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_keypair \
...
dolly delegate-stake --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_pubkey \
...
# Sign a stake delegation, assuming the authorized staker is held offline
sign_only="$(dolly deactivate-stake --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_keypair \
...
# Send the signed transaction on the cluster
dolly deactivate-stake --blockhash $nonce --nonce $nonce_account_pubkey --nonce-authority $offline_system_account_pubkey \
