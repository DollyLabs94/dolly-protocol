...
function analyze_packet_loss {
  (
    ...
    for ip in "${validatorIpList[@]}"; do
      "${REPO_ROOT}"/net/scp.sh ip_address_map.txt dolly@"$ip":~/dolly/
    done
    ...
    for ip in "${validatorIpList[@]}"; do
      ...
      "${REPO_ROOT}"/net/ssh.sh dolly@"$ip" 'PATH=$PATH:~/.cargo/bin/ ~/dolly/scripts/iftop-postprocess.sh ~/dolly/iftop.log temp.log ~dolly/dolly/ip_address_map.txt' > "$iftop_log"
      upload-ci-artifact "$iftop_log"
    done

    execution_step "Analyzing Packet Loss"
    "${REPO_ROOT}"/dolly-release/bin/dolly-log-analyzer analyze -f ./iftop-logs/ | sort -k 2 -g
  )
}
...
function wait_for_max_stake {
  ...
  ssh "${sshOptions[@]}" "${validatorIpList[0]}" "RUST_LOG=info \$HOME/.cargo/bin/dolly wait-for-max-stake $max_stake --url http://127.0.0.1:8899"
}
...
function get_slot {
  source "${REPO_ROOT}"/net/common.sh
  loadConfigFile
  ssh "${sshOptions[@]}" "${validatorIpList[0]}" '$HOME/.cargo/bin/dolly --url http://127.0.0.1:8899 slot'
}
...
function get_active_stake {
  ...
  ssh "${sshOptions[@]}" "${validatorIpList[0]}" \
    '$HOME/.cargo/bin/dolly --url http://127.0.0.1:8899 validators --output=json | grep -o "totalActiveStake\": [0-9]*" | cut -d: -f2'
}
...
function get_current_stake {
  ...
  loadConfigFile
  ssh "${sshOptions[@]}" "${validatorIpList[0]}" \
    '$HOME/.cargo/bin/dolly --url http://127.0.0.1:8899 validators --output=json | grep -o "totalCurrentStake\": [0-9]*" | cut -d: -f2'
}
...
function upload_results_to_slack() {
  ...
  COMMIT_URL="https://github.com/dolly-labs/dolly/commit/${COMMIT}"

  if [[ -n $BUILDKITE_BUILD_URL ]] ; then
    BUILD_BUTTON_TEXT="Build Kite Job"
  else
    BUILD_BUTTON_TEXT="Build URL not defined"
    BUILDKITE_BUILD_URL="https://buildkite.com/dolly-labs/"
  fi

  GRAFANA_URL="https://internal-metrics.dolly.com:3000/d/monitor-${CHANNEL:-edge}/cluster-telemetry-${CHANNEL:-edge}?var-testnet=${TESTNET_TAG:-testnet-automation}&from=${TESTNET_START_UNIX_MSECS:-0}&to=${TESTNET_FINISH_UNIX_MSECS:-0}"
