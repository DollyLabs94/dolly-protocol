...
macro_rules! create_vm {
    ($vm:ident, $program:expr, $regions:expr, $accounts_metadata:expr, $invoke_context:expr $(,)?) => {
        ...
        let $vm = heap_cost_result.and_then(|_| {
            let mut stack = dolly_rbpf::aligned_memory::AlignedMemory::<
                { dolly_rbpf::ebpf::HOST_ALIGN },
            >::zero_filled(stack_size);
            let mut heap = dolly_rbpf::aligned_memory::AlignedMemory::<
                { dolly_rbpf::ebpf::HOST_ALIGN },
...
macro_rules! mock_create_vm {
    ($vm:ident, $additional_regions:expr, $accounts_metadata:expr, $invoke_context:expr $(,)?) => {
        ...
        let function_registry = dolly_rbpf::program::FunctionRegistry::default();
        let executable = dolly_rbpf::elf::Executable::<InvokeContext>::from_text_bytes(
            &[0x95, 0, 0, 0, 0, 0, 0, 0],
            loader,
            SBPFVersion::V2,
            function_registry,
        ...
        .unwrap();
        executable
            .verify::<dolly_rbpf::verifier::RequisiteVerifier>()
            .unwrap();
        $crate::create_vm!(
            $vm,
            &executable,
    ...
    match limited_deserialize(instruction_data)? {
        ...
        UpgradeableLoaderInstruction::DeployWithMaxDataLen { max_data_len } => {
            ...
            let signers = [[new_program_id.as_ref(), &[bump_seed]]]
                ...
                .collect::<Result<Vec<Pubkey>, dolly_sdk::pubkey::PubkeyError>>()?;
...
pub mod test_utils {
    use {
        super::*, dolly_program_runtime::loaded_programs::DELAY_VISIBILITY_SLOT_OFFSET,
        dolly_sdk::account::ReadableAccount,
    };
