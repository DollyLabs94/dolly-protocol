...
#[deprecated(
    since = "1.8.0",
    note = "Please use `dolly_sdk::stake::state` or `dolly_program::stake::state` instead"
)]
pub use dolly_sdk::stake::state::*;
use {
...
pub fn delegate(
    ...
    if *vote_account.get_owner() != dolly_vote_program::id() {
...
pub(crate) fn deactivate_delinquent(
    ...
    if *delinquent_vote_account.get_owner() != dolly_vote_program::id() {
        return Err(InstructionError::IncorrectProgramId);
    }
    ...
    if *reference_vote_account.get_owner() != dolly_vote_program::id() {
        return Err(InstructionError::IncorrectProgramId);
    }
...
mod tests {
    ...
    fn test_authorized_authorize() {
        let staker = dolly_sdk::pubkey::new_rand();
    ...
    fn test_authorized_authorize_with_custodian() {
        ...
        let invalid_custodian = dolly_sdk::pubkey::new_rand();
        let mut authorized = Authorized::auto(&staker);
        let mut signers = HashSet::new();
        signers.insert(staker);
    ...
    fn test_dbg_stake_minimum_balance() {
        ...
        panic!(
            ...
            minimum_balance,
            minimum_balance as f64 / dolly_sdk::native_token::LAMPORTS_PER_SOL as f64
        );
    }
