...
local|tar|skip)
  ...
  dolly_cli=dolly
  dolly_gossip=dolly-gossip
  dolly_install=dolly-install
  ;;
*)
...
(
  set -x
  $dolly_cli --url http://"$sanityTargetIp":8899 validators
)
...
(
  ...
  $dolly_gossip --allow-private-addr spy --entrypoint "$sanityTargetIp:8001" \
...
if $installCheck && [[ -r update_manifest_keypair.json ]]; then
  echo "--- $sanityTargetIp: dolly-install test"
