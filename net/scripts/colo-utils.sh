...
declare -r SOLANA_LOCK_FILE="/home/dolly/.dolly.lock"
...
colo_instance_run() {
  ...
  declare OUT
  set +e
  OUT=$(ssh -l dolly -o "StrictHostKeyChecking=no" -o "ConnectTimeout=3" -n "${IP}" "${CMD}" 2>&1)
  declare RC=$?
  set -e
  while read -r LINE; do
...
SSH_AUTHORIZED_KEYS='$("${__colo_here}"/add-datacenter-dolly-user-authorized_keys.sh 2> /dev/null)'
