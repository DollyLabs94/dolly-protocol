...
Operate a configured testnet
 ...
 sanity/start-specific options:
   ...
   -o noInstallCheck    - Skip dolly-install sanity
 ...
 update-specific options:
   --platform linux|osx|windows       - Deploy the tarball using 'dolly-install deploy ...' for the
...
startCommon() {
  ...
  local remoteSolanaHome="${remoteHome}/dolly"
  ...
  [[ -z "$externalNodeSshKey" ]] || ssh-copy-id -f -i "$externalNodeSshKey" "${sshOptions[@]}" "dolly@$ipAddress"
...
deployBootstrapValidator() {
  ...
  tar)
    rsync -vPrc -e "ssh ${sshOptions[*]}" "$SOLANA_ROOT"/dolly-release/bin/* "$ipAddress:$remoteCargoBin/"
    rsync -vPrc -e "ssh ${sshOptions[*]}" "$SOLANA_ROOT"/dolly-release/version.yml "$ipAddress:~/"
    ;;
  local)
...
startBootstrapLeader() {
  ...
  (
    ...
    ssh "${sshOptions[@]}" -n "$ipAddress" \
      "./dolly/net/remote/remote-node.sh \
         $deployMethod \
         bootstrap-validator \
         $entrypointIp \
...
startNode() {
  ...
  (
    ...
    if [[ $nodeType = blockstreamer ]] && [[ -n $letsEncryptDomainName ]]; then
      ...
      ssh "${sshOptions[@]}" -n "$ipAddress" \
        "sudo -H /certbot-restore.sh $letsEncryptDomainName maintainers@dollylabs.com"
...
startClient() {
  ...
  (
    ...
    ssh "${sshOptions[@]}" -f "$ipAddress" \
      "./dolly/net/remote/remote-client.sh $deployMethod $entrypointIp \
...
startClients() {
  for ((i=0; i < "$numClients" && i < "$numClientsRequested"; i++)) do
    if [[ $i -lt "$numBenchTpsClients" ]]; then
      startClient "${clientIpList[$i]}" "dolly-bench-tps" "$i"
    else
...
sanity() {
  ...
  (
    ...
    ssh "${sshOptions[@]}" "$bootstrapLeader" \
      "./dolly/net/remote/remote-sanity.sh $bootstrapLeader $sanityExtraArgs \"$RUST_LOG\""
  ) || ok=false
  $ok || exit 1
  ...
  if [[ -z $skipBlockstreamerSanity && -n $blockstreamer ]]; then
    ...
    (
      ...
      ssh "${sshOptions[@]}" "$blockstreamer" \
        "./dolly/net/remote/remote-sanity.sh $blockstreamer $sanityExtraArgs \"$RUST_LOG\""
    ) || ok=false
    $ok || exit 1
  fi
...
deployUpdate() {
  ...
  for updatePlatform in $updatePlatforms; do
    echo "--- Deploying dolly-install update: $updatePlatform"
    (
      set -x

      scripts/dolly-install-update-manifest-keypair.sh "$updatePlatform"

      timeout 30s scp "${sshOptions[@]}" \
        update_manifest_keypair.json "$bootstrapLeader:dolly/update_manifest_keypair.json"

      # shellcheck disable=SC2029 # remote-deploy-update.sh args are expanded on client side intentionally
      ssh "${sshOptions[@]}" "$bootstrapLeader" \
        "./dolly/net/remote/remote-deploy-update.sh $releaseChannel $updatePlatform"
    ) || ok=false
    $ok || exit 1
  done
}
...
prepareDeploy() {
  ...
  tar)
    if [[ -n $releaseChannel ]]; then
      ...
      rm -f "$SOLANA_ROOT"/dolly-release.tar.bz2
      declare updateDownloadUrl=https://release.dolly.com/"$releaseChannel"/dolly-release-x86_64-unknown-linux-gnu.tar.bz2
      (
        set -x
        curl -L -I "$updateDownloadUrl"
        curl -L --retry 5 --retry-delay 2 --retry-connrefused \
          -o "$SOLANA_ROOT"/dolly-release.tar.bz2 "$updateDownloadUrl"
      )
      tarballFilename="$SOLANA_ROOT"/dolly-release.tar.bz2
    fi
    (
      set -x
      rm -rf "$SOLANA_ROOT"/dolly-release
      cd "$SOLANA_ROOT"; tar jfxv "$tarballFilename"
      cat "$SOLANA_ROOT"/dolly-release/version.yml
    )
    ;;
  local)
    if $doBuild; then
  ...
  *)
  ...
  if [[ -n $deployIfNewer ]]; then
    ...
    if ! diff -q current-version.yml "$SOLANA_ROOT"/dolly-release/version.yml; then
...
deploy() {
  ...
  if ! $waitForNodeInit; then
    ...
    for ipAddress in "${validatorIpList[@]}" "${blockstreamerIpList[@]}"; do
      ...
      ssh "${sshOptions[@]}" -n "$ipAddress" \
        "./dolly/net/remote/remote-node-wait-init.sh $((600 - timeWaited))"
    done
  fi
  ...
  tar)
    networkVersion="$(
      (
        set -o pipefail
        grep "^commit: " "$SOLANA_ROOT"/dolly-release/version.yml | head -n1 | cut -d\  -f2
      ) || echo "tar-unknown"
    )"
    ;;
  local)
...
stopNode() {
  ...
  (
    ...
    ssh "${sshOptions[@]}" "$ipAddress" "PS4=\"$PS4\" ./dolly/net/remote/cleanup.sh"
  ) >> "$logFile" 2>&1 &
...
logs)
  ...
  fetchRemoteLog() {
    ...
    (
      set -x
      timeout 30s scp "${sshOptions[@]}" \
        "$ipAddress":dolly/"$log".log "$netLogDir"/remote-"$log"-"$ipAddress".log
...
netem)
  if [[ -n $netemConfigFile ]]; then
    ...
    if [[ $netemCommand = "add" ]]; then
      for ipAddress in "${validatorIpList[@]}"; do
        remoteHome=$(remoteHomeDir "$ipAddress")
        remoteSolanaHome="${remoteHome}/dolly"
        "$here"/scp.sh "$netemConfigFile" dolly@"$ipAddress":"$remoteSolanaHome"
      done
    fi
    for i in "${!validatorIpList[@]}"; do
      "$here"/ssh.sh dolly@"${validatorIpList[$i]}" 'dolly/scripts/net-shaper.sh' \
      "$netemCommand" ~dolly/dolly/"$remoteNetemConfigFile" "${#validatorIpList[@]}" "$i"
    done
  else
    ...
    for ipAddress in "${validatorIpList[@]}"; do
      "$here"/ssh.sh dolly@"$ipAddress" 'dolly/scripts/netem.sh delete < dolly/netem.cfg || true'
    done

    # Start netem on required nodes
    for ((i=0; i<num_nodes; i++ )); do :
      "$here"/ssh.sh dolly@"${validatorIpList[$i]}" "echo $netemConfig > dolly/netem.cfg; dolly/scripts/netem.sh add \"$netemConfig\""
    done
  fi
  ;;
*)
