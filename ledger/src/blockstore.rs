...
use {
    ...
    dolly_accounts_db::hardened_unpack::{
        unpack_genesis_archive, MAX_GENESIS_ARCHIVE_UNPACKED_SIZE,
    },
    dolly_entry::entry::{create_ticks, Entry},
    ...
    dolly_storage_proto::{StoredExtendedRewards, StoredTransactionStatusMeta},
...
///
/// `dolly_core::completed_data_sets_service::CompletedDataSetsService` is the main receiver of
...
impl Blockstore {
        ...
        if slot_meta.is_full() {
            ...
            if !slot_entries.is_empty() {
                ...
                let slot_transaction_iterator = slot_entries
                    ...
                    .flat_map(|entry| {
                        if populate_entries {
                            entries.push(dolly_transaction_status::EntrySummary {
    ...
    fn cleanup_old_entries(&self) -> Result<()> {
        ...
        // https://github.com/dolly-labs/dolly/blob/bc2b372/ledger/src/blockstore.rs#L2130-L2137
    ...
    #[deprecated(
        since = "1.18.0",
        note = "Please use `dolly_ledger::blockstore::Blockstore::max_root()` instead"
    )]
    ...
    /// https://github.com/dolly-labs/dolly/pull/2253
    ///
    /// Arguments:
...
fn adjust_ulimit_nofile(enforce_ulimit_nofile: bool) -> Result<()> {
    ...
    // https://docs.dollylabs.com/operations/guides/validator-start#increased-memory-mapped-files-limit
...
pub mod tests {
    use {
        ...
        dolly_account_decoder::parse_token::UiTokenAmount,
        dolly_entry::entry::{next_entry, next_entry_mut},
        dolly_runtime::bank::{Bank, RewardType},
        dolly_sdk::{
        ...
        dolly_storage_proto::convert::generated,
        dolly_transaction_status::{
    ...
    */
    ...
    fn test_persist_transaction_status() {
        ...
        // insert value
        let status = TransactionStatusMeta {
            status: dolly_sdk::transaction::Result::<()>::Err(TransactionError::AccountNotFound),
            fee: 5u64,
        ...
        // insert value
        let status = TransactionStatusMeta {
            status: dolly_sdk::transaction::Result::<()>::Ok(()),
            fee: 9u64,
            pre_balances: pre_balances_vec.clone(),
            post_balances: post_balances_vec.clone(),
            inner_instructions: Some(inner_instructions_vec.clone()),
    ...
    fn do_test_lowest_cleanup_slot_and_special_cfs(simulate_blockstore_cleanup_service: bool) {
        ...
        let address0 = dolly_sdk::pubkey::new_rand();
        let address1 = dolly_sdk::pubkey::new_rand();
        blockstore
            .write_transaction_status(
                lowest_cleanup_slot,
                signature1,
    ...
    fn test_map_transactions_to_statuses() {
        ...
        for x in 0..4 {
            ...
            let status = TransactionStatusMeta {
                status: dolly_sdk::transaction::Result::<()>::Err(
    ...
    fn test_rewards_protobuf_backward_compatibility() {
        ...
        let rewards: Rewards = (0..100)
            .map(|i| Reward {
                pubkey: dolly_sdk::pubkey::new_rand().to_string(),
                lamports: 42 + i,
    ...
    fn make_large_tx_entry(num_txs: usize) -> Entry {
        let txs: Vec<_> = (0..num_txs)
            .map(|_| {
                ...
                dolly_sdk::system_transaction::transfer(&keypair0, &to, 1, Hash::default())
            })
            .collect();
