use {
    ...
    dolly_config_program::{config_instruction, get_config_data, ConfigState},
...
fn load_release_target(release_dir: &Path) -> Result<String, String> {
    ...
    version_yml.push("dolly-release");
...
pub fn init(
    ...
    let config = {
        // Write new config file only if different, so that running |dolly-install init|
...
fn github_release_download_url(release_semver: &str) -> String {
    format!(
        "https://github.com/dolly-labs/dolly/releases/download/v{}/dolly-release-{}.tar.bz2",
        release_semver,
...
fn release_channel_download_url(release_channel: &str) -> String {
    format!(
        "https://release.dolly.com/{}/dolly-release-{}.tar.bz2",
        release_channel,
        crate::build_env::TARGET
    )
}
...
fn release_channel_version_url(release_channel: &str) -> String {
    format!(
        "https://release.dolly.com/{}/dolly-release-{}.yml",
        release_channel,
        crate::build_env::TARGET
    )
}
...
fn check_for_newer_github_release(
    ...
    while page == 1 || releases.len() == PER_PAGE {
        let url = reqwest::Url::parse_with_params(
            "https://api.github.com/repos/dolly-labs/dolly/releases",
            &[
...
pub fn init_or_update(config_file: &str, is_init: bool, check_only: bool) -> Result<bool, String> {
        ...
        match explicit_release {
            ...
            ExplicitRelease::Channel(release_channel) => {
                ...
                let current_release_version_yml =
                    release_dir.join("dolly-release").join("version.yml");
    ...
    symlink_dir(
        release_dir.join("dolly-release"),
