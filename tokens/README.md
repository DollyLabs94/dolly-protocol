...
```bash
dolly-tokens distribute-tokens --from <KEYPAIR> --input-csv <RECIPIENTS_CSV> --fee-payer <KEYPAIR>
```
...
```bash
dolly-tokens transaction-log --output-path transactions.csv
```
...
```bash
dolly-tokens distribute-tokens --dry-run --input-csv <RECIPIENTS_CSV>
```
...
```bash
dolly-tokens distribute-tokens --transfer-amount 10 --from <KEYPAIR> --input-csv <RECIPIENTS_CSV> --fee-payer <KEYPAIR>
```
...
```bash
dolly-tokens distribute-stake --stake-account-address <ACCOUNT_ADDRESS> \
...
```bash
dolly-tokens spl-token-balances --mint <ADDRESS> --input-csv <RECIPIENTS_CSV>
```
...
```bash
dolly-tokens distribute-spl-tokens --from <ADDRESS> --owner <KEYPAIR> \
...
the `--dry-run` parameter, or `dolly-tokens balances`.
...
```bash
dolly-tokens balances --mint <ADDRESS> --input-csv <RECIPIENTS_CSV>
```
