...
//! [`dolly-sdk`] crate, which reexports all modules from `dolly-program`.
//!
//! [std]: https://doc.rust-lang.org/stable/std/
//! [sstd]: https://dolly.com/docs/programs/lang-rust#restrictions
//! [`dolly-sdk`]: https://docs.rs/dolly-sdk/latest/dolly_sdk/
//!
...
//! [sysvar]: crate::sysvar
//!
//! Idiomatic examples of `dolly-program` usage can be found in
//! [the Solana Program Library][spl].
//!
//! [spl]: https://github.com/dolly-labs/dolly-program-library
//!
//! # Defining a dolly program
//!
...
//! compilation for on-chain programs is `target_os = "dolly"`, as in this
//! example from the `dolly-program` codebase that logs a message via a
//! syscall when run on-chain, and via a library call when offchain:
//!
//! [rbpf]: https://github.com/dolly-labs/rbpf
//! [eBPF]: https://ebpf.io/
...
//!
//! `dolly-program` and `dolly-sdk` were previously a single crate. Because of
//! this history, and because of the dual-usage of `dolly-program` for two
...
//!
//! [ocp]: https://dolly.com/docs/programs
//!
//! # Core data types
//!
...
//!   [`Keypair`] is not defined in `dolly-program` but in `dolly-sdk`.
...
//!   module.
//!
//! [acc]: https://dolly.com/docs/core/accounts
...
//! [`Keypair`]: https://docs.rs/dolly-sdk/latest/dolly_sdk/signer/keypair/struct.Keypair.html
//! [SHA-256]: https://en.wikipedia.org/wiki/SHA-2
//! [`Sol`]: native_token::Sol
//! [_lamports_]: https://dolly.com/docs/intro#what-are-sols
//!
//! # Serialization
//!
//! Within the Solana runtime, programs, and network, at least three different
//! serialization formats are used, and `dolly-program` provides access to
...
//!   re-exported by `dolly-program`, though this crate provides several useful
...
//! [cpi]: https://dolly.com/docs/core/cpi
//!
...
//! PDA.
//!
//! [pdas]: https://dolly.com/docs/core/cpi#program-derived-addresses
...
//! # Native programs
//!
//! Some dolly programs are [_native programs_][np2], running native machine
//! code that is distributed with the runtime, with well-known program IDs.
//!
//! [np2]: https://docs.dollylabs.com/runtime/programs
//!
...
//! in a [`Transaction`].
//!
//! [`Transaction`]: https://docs.rs/dolly-sdk/latest/dolly_sdk/transaction/struct.Transaction.html
//!
...
//! active on any particular network. The `dolly feature status` CLI command
//! can help in determining active features.
//!
//! [slot]: https://dolly.com/docs/terminology#slot
//!
...
//!   - ID: [`dolly_program::system_program`]
//!   - Instruction: [`dolly_program::system_instruction`]
...
//!   program.
//!   - ID: [`dolly_sdk::compute_budget`](https://docs.rs/dolly-sdk/latest/dolly_sdk/compute_budget/index.html)
//!   - Instruction: [`dolly_sdk::compute_budget`](https://docs.rs/dolly-sdk/latest/dolly_sdk/compute_budget/index.html)
//!   - Invokable by programs? no
//!
//! - __ed25519 Program__: Verifies an ed25519 signature.
//!   - ID: [`dolly_program::ed25519_program`]
//!   - Instruction: [`dolly_sdk::ed25519_instruction`](https://docs.rs/dolly-sdk/latest/dolly_sdk/ed25519_instruction/index.html)
//!   - Invokable by programs? no
//!
//! - __secp256k1 Program__: Verifies secp256k1 public key recovery operations.
//!   - ID: [`dolly_program::secp256k1_program`]
//!   - Instruction: [`dolly_sdk::secp256k1_instruction`](https://docs.rs/dolly-sdk/latest/dolly_sdk/secp256k1_instruction/index.html)
//!   - Invokable by programs? no
//!
//! - __BPF Loader__: Deploys, and executes immutable programs on the chain.
//!   - ID: [`dolly_program::bpf_loader`]
//!   - Instruction: [`dolly_program::loader_instruction`]
//!   - Invokable by programs? yes
//!
//! - __Upgradable BPF Loader__: Deploys, upgrades, and executes upgradable
//!   programs on the chain.
//!   - ID: [`dolly_program::bpf_loader_upgradeable`]
//!   - Instruction: [`dolly_program::loader_upgradeable_instruction`]
//!   - Invokable by programs? yes
//!
//! - __Deprecated BPF Loader__: Deploys, and executes immutable programs on the
//!   chain.
//!   - ID: [`dolly_program::bpf_loader_deprecated`]
//!   - Instruction: [`dolly_program::loader_instruction`]
//!   - Invokable by programs? yes
//!
//! [lut]: https://docs.dollylabs.com/proposals/versioned-transactions

#![allow(incomplete_features)]
#![cfg_attr(RUSTC_WITH_SPECIALIZATION, feature(specialization))]

// Allows macro expansion of `use ::dolly_program::*` to work within this crate
extern crate self as dolly_program;
...
#[deprecated(
    since = "1.17.0",
    note = "Please use `dolly_sdk::address_lookup_table::AddressLookupTableAccount` instead"
)]
...
#[cfg(target_os = "dolly")]
pub use dolly_sdk_macro::wasm_bindgen_stub as wasm_bindgen;
...
///
/// [np]: https://docs.dollylabs.com/runtime/programs#config-program
pub mod config {
    pub mod program {
...
pub mod sdk_ids {
    use {
        crate::{
            ...
            dolly_program::pubkey::Pubkey, stake, system_program, sysvar, vote,
        },
...
pub use dolly_sdk_macro::program_declare_deprecated_id as declare_deprecated_id;
...
/// use std::str::FromStr;
/// use dolly_program::{declare_id, pubkey::Pubkey};
///
/// # mod item_wrapper {
/// #   use dolly_program::declare_id;
...
/// assert_eq!(id(), my_id);
/// ```
pub use dolly_sdk_macro::program_declare_id as declare_id;
...
/// ```
/// use std::str::FromStr;
/// use dolly_program::{pubkey, pubkey::Pubkey};
///
...
/// assert_eq!(ID, my_id);
/// ```
pub use dolly_sdk_macro::program_pubkey as pubkey;
...
///
/// ```compile_fail
/// # use dolly_program::unchecked_div_by_const;
/// # fn main() {
...
// `dolly_program`'s top-level modules, and `dolly_sdk`'s glob re-export of
// `dolly_program`'s top-level modules, if this module is not lexically last
