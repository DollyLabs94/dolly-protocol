...
use {
    ...
    dolly_sdk::signer::{EncodableKey, SeedDerivable},
...
fn main() -> Result<(), Box<dyn error::Error>> {
    let matches = app(dolly_version::version!())
        .try_get_matches()
    ...
    match subcommand {
        ("new", matches) => {
            ...
            let outfile = if matches.is_present("outfile") {
                ...
                path.extend([".config", "dolly", key_type.default_file_name()]);
...
mod tests {
    ...
    fn test_arguments() {
        ...
        app(dolly_version).debug_assert();
    }
    ...
    fn test_pubkey() {
        ...
        process_test_command(&["dolly-keygen", "pubkey", "elgamal", &keypair_path]).unwrap();
    }
}
