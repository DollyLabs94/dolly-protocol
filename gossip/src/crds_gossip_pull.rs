...
use {
    ...
    dolly_bloom::bloom::{Bloom, ConcurrentBloom},
    dolly_sdk::{
        hash::{hash, Hash},
...
impl dolly_sdk::sanitize::Sanitize for CrdsFilter {
    fn sanitize(&self) -> std::result::Result<(), dolly_sdk::sanitize::SanitizeError> {
...
pub(crate) mod tests {
    use {
        ...
        dolly_perf::test_tx::new_test_vote_tx,
        dolly_sdk::{
    ...
    fn test_new_pull_request() {
        ...
        let new = ContactInfo::new_localhost(&dolly_sdk::pubkey::new_rand(), now);
        ping_cache
            .lock()
            .unwrap()
        ...
        let offline = ContactInfo::new_localhost(&dolly_sdk::pubkey::new_rand(), now);
    ...
    fn test_new_mark_creation_time() {
        ...
        let old = ContactInfo::new_localhost(&dolly_sdk::pubkey::new_rand(), 0);
        ...
        crds.insert(old.clone(), now, GossipRoute::LocalMessage)
            .unwrap();
        let new = ContactInfo::new_localhost(&dolly_sdk::pubkey::new_rand(), 0);
    ...
    fn test_process_pull_request_response() {
        ...
        let new = ContactInfo::new_localhost(&dolly_sdk::pubkey::new_rand(), 1);
        ping_cache.mock_pong(*new.pubkey(), new.gossip().unwrap(), Instant::now());
        let new = CrdsValue::new_unsigned(CrdsData::LegacyContactInfo(new));
        node_crds.insert(new, 0, GossipRoute::LocalMessage).unwrap();

        let mut dest_crds = Crds::default();
        let new_id = dolly_sdk::pubkey::new_rand();
    ...
    fn test_gossip_purge() {
        ...
        let mut node_crds = Crds::default();
        let entry = CrdsValue::new_unsigned(CrdsData::LegacyContactInfo(
            ContactInfo::new_localhost(&dolly_sdk::pubkey::new_rand(), 0),
        ));
    ...
    fn test_process_pull_response() {
        ...
        let peer_pubkey = dolly_sdk::pubkey::new_rand();
