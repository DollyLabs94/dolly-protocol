...
impl TryFrom<AccountSharedData> for VoteAccountInner {
    type Error = Error;
    fn try_from(account: AccountSharedData) -> Result<Self, Self::Error> {
        if !dolly_vote_program::check_id(account.owner()) {
...
mod tests {
    use {
        ...
        bincode::Options,
        rand::Rng,
        dolly_sdk::{pubkey::Pubkey, sysvar::clock::Clock},
        dolly_vote_program::vote_state::{VoteInit, VoteStateVersions},
