use {
    ...
    itertools::Itertools,
    log::*,
    dolly_accounts_db::accounts_index::{AccountIndex, AccountSecondaryIndexes},
    dolly_clap_utils::{
        input_parsers::{pubkey_of, pubkeys_of, value_of},
        input_validators::normalize_to_url_if_moniker,
    },
    dolly_core::consensus::tower_storage::FileTowerStorage,
    dolly_faucet::faucet::run_local_faucet_with_port,
    dolly_rpc::{
...
fn main() {
    ...
    let version = dolly_version::version!();
    ...
    dolly_runtime::snapshot_utils::remove_tmp_snapshot_archives(&ledger_path);
    ...
    info!("{} {}", crate_name!(), dolly_version::version!());
    ...
    let cli_config = if let Some(config_file) = matches.value_of("config_file") {
        dolly_cli_config::Config::load(config_file).unwrap_or_default()
    } else {
        dolly_cli_config::Config::default()
    };
    ...
    let dynamic_port_range = matches.value_of("dynamic_port_range").map(|port_range| {
        dolly_net_utils::parse_port_range(port_range).unwrap_or_else(|| {
            eprintln!("Failed to parse --dynamic-port-range");
            exit(1);
        })
    });
    let bind_address = matches.value_of("bind_address").map(|bind_address| {
        dolly_net_utils::parse_host(bind_address).unwrap_or_else(|err| {
    ...
    if TestValidatorGenesis::ledger_exists(&ledger_path) {
        ...
        println_name_value(
            "\nNotice!",
            "No wallet available. `dolly airdrop` localnet SOL after creating one\n",
        );
    }
