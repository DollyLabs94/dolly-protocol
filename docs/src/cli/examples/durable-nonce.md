...
[`recent_blockhash`](https://dolly.com/docs/core/transactions#recent-blockhash).
...
```bash
dolly-keygen new -o nonce-keypair.json
dolly create-nonce-account nonce-keypair.json 1
```
...
> [Full usage documentation](../usage.md#dolly-create-nonce-account)
...
```bash
dolly nonce nonce-keypair.json
```
...
> [Full usage documentation](../usage.md#dolly-get-nonce)
...
```bash
dolly new-nonce nonce-keypair.json
```
...
> [Full usage documentation](../usage.md#dolly-new-nonce)
...
```bash
dolly nonce-account nonce-keypair.json
```
...
> [Full usage documentation](../usage.md#dolly-nonce-account)
...
```bash
dolly withdraw-from-nonce-account nonce-keypair.json ~/.config/dolly/id.json 0.5
```
...
> [Full usage documentation](../usage.md#dolly-withdraw-from-nonce-account)
...
```bash
dolly authorize-nonce-account nonce-keypair.json nonce-authority.json
```
...
> [Full usage documentation](../usage.md#dolly-authorize-nonce-account)
...
```bash
$ dolly-keygen new -o alice.json
$ dolly-keygen new -o nonce.json
$ dolly-keygen new -o bob.json
```
...
```bash
$ dolly create-nonce-account -k alice.json nonce.json 0.1
...
```bash
$ dolly transfer -k alice.json --blockhash expiredDTaxfagttWjQweib42b6ZHADSx94Tw8gHx11 bob.json 0.01
[2020-01-02T18:48:28.462911000Z ERROR dolly_cli::cli] Io(Custom { kind: Other, error: "Transaction \"33gQQaoPc9jWePMvDAeyJpcnSPiGUAdtVg8zREWv4GiKjkcGNufgpcbFyRKRrA25NkgjZySEeKue5rawyeH5TzsV\" failed: None" })
...
```bash
$ dolly nonce-account nonce.json
balance: 0.1 SOL
minimum balance required: 0.00136416 SOL
...
```bash
$ dolly transfer -k alice.json --blockhash F7vmkY3DTaxfagttWjQweib42b6ZHADSx94Tw8gHx3W7 --nonce nonce.json bob.json 0.01
