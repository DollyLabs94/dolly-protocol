...
```bash
dolly airdrop 1 <RECIPIENT_ACCOUNT_ADDRESS> --url https://api.devnet.dolly.com
```
...
```bash
dolly confirm -v <TRANSACTION_SIGNATURE>
```
...
```bash
dolly balance <ACCOUNT_ADDRESS> --url https://api.devnet.dolly.com
```
...
```bash
dolly-keygen new --no-passphrase --no-outfile
```
...
```bash
dolly transfer --from <KEYPAIR> <RECIPIENT_ACCOUNT_ADDRESS> 0.5 --allow-unfunded-recipient --url https://api.devnet.dolly.com --fee-payer <KEYPAIR>
```
...
Confirm the updated balances with `dolly balance`:

```bash
dolly balance <ACCOUNT_ADDRESS> --url http://api.devnet.dolly.com
```
...
```bash
$ dolly-keygen new --outfile my_dolly_wallet.json   # Creating my first wallet, a file system wallet
Generating a new keypair
For added security, enter a passphrase (empty for no passphrase):
Wrote new keypair to my_dolly_wallet.json
==========================================================================
...
$ dolly airdrop 1 DYw8jCTfwHNRJhhmFcbXvVDTqWMEVFBX6ZKUmG5CNSKK --url https://api.devnet.dolly.com  # Airdropping 1 SOL to my wallet's address/pubkey
Requesting airdrop of 1 SOL from 35.233.193.70:9900
1 SOL

$ dolly balance DYw8jCTfwHNRJhhmFcbXvVDTqWMEVFBX6ZKUmG5CNSKK --url https://api.devnet.dolly.com # Check the address's balance
1 SOL

$ dolly-keygen new --no-outfile  # Creating a second wallet, a paper wallet
Generating a new keypair
For added security, enter a passphrase (empty for no passphrase):
====================================================================
...
$ dolly transfer --from my_dolly_wallet.json 7S3P4HxJpyyigGzodYwHtCxZyUQe9JiBMHyRWXArAaKv 0.5 --allow-unfunded-recipient --url https://api.devnet.dolly.com --fee-payer my_dolly_wallet.json  # Transferring tokens to the public address of the paper wallet
3gmXvykAd1nCQQ7MjosaHLf69Xyaqyq1qw2eu1mgPyYXd5G4v1rihhg1CiRw35b9fHzcftGKKEu4mbUeXY2pEX2z  # This is the transaction signature

$ dolly balance DYw8jCTfwHNRJhhmFcbXvVDTqWMEVFBX6ZKUmG5CNSKK --url https://api.devnet.dolly.com
...
$ dolly balance 7S3P4HxJpyyigGzodYwHtCxZyUQe9JiBMHyRWXArAaKv --url https://api.devnet.dolly.com
...
with the `dolly transfer` command:

```bash
dolly transfer --from <KEYPAIR> <RECIPIENT_ACCOUNT_ADDRESS> <AMOUNT> --fee-payer <KEYPAIR>
```

Confirm the updated balances with `dolly balance`:

```bash
dolly balance <ACCOUNT_ADDRESS>
```
