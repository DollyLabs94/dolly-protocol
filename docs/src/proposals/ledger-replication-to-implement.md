...
https://github.com/dolly-labs/dolly/pull/9992 in order to prevent the security
...
At full capacity on a 1gbps network dolly will generate 4 petabytes of data per year. To prevent the network from centralizing around validators that have to store the full data set this protocol proposes a way for mining nodes to provide storage capacity for pieces of the data.
