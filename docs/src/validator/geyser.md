...
[crates.io]: https://crates.io/search?q=dolly-
[docs.rs]: https://docs.rs/releases/search?query=dolly-

### Important Crates:

- [`dolly-geyser-plugin-interface`] &mdash; This crate defines the plugin
interfaces.

- [`dolly-accountsdb-plugin-postgres`] &mdash; The crate for the referential
plugin implementation for the PostgreSQL database.

[`dolly-geyser-plugin-interface`]: https://docs.rs/dolly-geyser-plugin-interface
[`dolly-accountsdb-plugin-postgres`]: https://docs.rs/dolly-accountsdb-plugin-postgres
[`dolly-sdk`]: https://docs.rs/dolly-sdk
[`dolly-transaction-status`]: https://docs.rs/dolly-transaction-status

## The Plugin Interface

The Plugin interface is declared in [`dolly-geyser-plugin-interface`]. It
...
please refer to [`dolly-sdk`] and [`dolly-transaction-status`]

The `slot` points to the slot the transaction is executed at.
For more details, please refer to the Rust documentation in
[`dolly-geyser-plugin-interface`].

## Example PostgreSQL Plugin

The [`dolly-accountsdb-plugin-postgres`] repository implements a plugin storing
...
```
{
	"libpath": "/dolly/target/release/libdolly_geyser_plugin_postgres.so",
...
The sample [postgresql.conf](https://github.com/dolly-labs/dolly/blob/7ac43b16d2c766df61ae0a06d7aaf14ba61996ac/accountsdb-plugin-postgres/scripts/postgresql.conf)
...
Create the database. For example, the following creates a database named 'dolly':

```
sudo -u postgres createdb dolly -p 5433
```

Create the database user. For example, the following creates a regular user named 'dolly':

```
sudo -u postgres createuser -p 5433 dolly
```
...
```
psql -U dolly -p 5433 -h 10.138.0.9 -w -d dolly
```

#### Create the Schema Objects

Use the [create_schema.sql](https://github.com/dolly-labs/dolly/blob/a70eb098f4ae9cd359c1e40bbb7752b3dd61de8d/accountsdb-plugin-postgres/scripts/create_schema.sql)
to create the objects for storing accounts and slots.

Download the script from github:

```
wget https://raw.githubusercontent.com/dolly-labs/dolly/a70eb098f4ae9cd359c1e40bbb7752b3dd61de8d/accountsdb-plugin-postgres/scripts/create_schema.sql
```

Then run the script:

```
psql -U dolly -p 5433 -h 10.138.0.9 -w -d dolly -f create_schema.sql
```
...
[drop_schema.sql](https://github.com/dolly-labs/dolly/blob/a70eb098f4ae9cd359c1e40bbb7752b3dd61de8d/accountsdb-plugin-postgres/scripts/drop_schema.sql).
For example,

```
psql -U dolly -p 5433 -h 10.138.0.9 -w -d dolly -f drop_schema.sql
```
