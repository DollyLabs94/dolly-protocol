...
First, setup Rust, Cargo and system packages as described in the Solana [README](https://github.com/dolly-labs/dolly#1-install-rustc-cargo-and-rustfmt)
...
The demo code is sometimes broken between releases as we add new low-level features, so if this is your first time running the demo, you'll improve your odds of success if you check out the [latest release](https://github.com/dolly-labs/dolly/releases) before proceeding:
...
- To enable `info` everywhere and `debug` only in the dolly::banking_stage module:

  ```bash
  export RUST_LOG=dolly=info,dolly::banking_stage=debug
  ```

- To enable SBF program logging:

  ```bash
  export RUST_LOG=dolly_bpf_loader=trace
  ```
...
You can also attach to a running process with GDB. The leader's process is named _dolly-validator_:
...
```bash
NDEBUG=1 ./multinode-demo/bench-tps.sh --entrypoint entrypoint.devnet.dolly.com:8001 --faucet api.devnet.dolly.com:9900 --duration 60 --tx_count 50
```

You can observe the effects of your client's transactions on our [metrics dashboard](https://metrics.dolly.com:3000/d/monitor/cluster-telemetry?var-testnet=devnet)
