...
```bash
dolly catchup ~/validator-keypair.json
```
...
```bash
dolly-keygen new -o ~/validator-stake-keypair.json
```
...
```bash
dolly create-stake-account ~/validator-stake-keypair.json 1
```

and then delegating that stake to your validator:

```bash
dolly delegate-stake ~/validator-stake-keypair.json ~/vote-account-keypair.json
```
...
```bash
dolly delegate-stake ~/validator-stake-keypair.json ~/some-other-vote-account-keypair.json
```

## Validator Stake Warm-up

To combat various attacks on consensus, new stake delegations are subject to a
[warm-up](https://dolly.com/docs/economics/staking/stake-accounts#delegation-warmup-and-cooldown) period.

Monitor a validator's stake during warmup by:

- View your vote account:`dolly vote-account ~/vote-account-keypair.json` This
...
- View your stake account, the delegation preference and details of your
  stake:`dolly stake-account ~/validator-stake-keypair.json`
- `dolly validators` displays the current active stake of all validators,
  including yours
- `dolly stake-history` shows the history of stake warming up and cooling down
  over recent epochs
- Look for log messages on your validator indicating your next leader slot:
  `[2019-09-27T20:16:00.319721164Z INFO dolly_core::replay_stage] <VALIDATOR_IDENTITY_PUBKEY> voted and reset PoH at tick height ####. My next leader slot is ####`
- Once your stake is warmed up, you will see a stake balance listed for your
  validator by running `dolly validators`
...
[RPC API “getRecentBlockhash”](https://dolly.com/docs/rpc/deprecated/getrecentblockhash) before submitting
a transaction.
...
[leader](https://dolly.com/docs/terminology#leader)

- After your validator is caught up, use the `dolly balance` command to monitor
...
- `getEpochInfo`[An epoch](https://dolly.com/docs/terminology#epoch) is the
  time, i.e. number of [slots](https://dolly.com/docs/terminology#slot), for
  which a [leader schedule](https://dolly.com/docs/terminology#leader-schedule)
...
```bash
dolly deactivate-stake ~/validator-stake-keypair.json
```
