...
### Primary Validator
The following additional `dolly-validator` parameters are required to enable
...
following `dolly-validator` command-line argument changes:
...
```bash
$ dolly-validator wait-for-restart-window --identity validator-keypair.json \
  && dolly-validator set-identity validator-keypair.json
```
...
Monitoring of your choosing can invoke the `dolly-validator set-identity
