...
The dolly cli includes `get` and `set` configuration commands to automatically
set the `--url` argument for cli commands. For example:

```bash
dolly config set --url http://api.devnet.dolly.com
```
...
```bash
dolly transaction-count
```

View the [metrics dashboard](https://metrics.dolly.com:3000/d/monitor/cluster-telemetry) for more
detail on cluster activity.

## Enabling CUDA

If your machine has a GPU with CUDA installed \(Linux-only currently\), include
the `--cuda` argument to `dolly-validator`.

When your validator is started look for the following log message to indicate
that CUDA is enabled: `"[<timestamp> dolly::validator] CUDA is enabled"`
...
```bash
sudo bash -c "cat >/etc/sysctl.d/21-dolly-validator.conf <<EOF
...
```bash
sudo sysctl -p /etc/sysctl.d/21-dolly-validator.conf
```
...
```bash
sudo bash -c "cat >/etc/security/limits.d/90-dolly-nofiles.conf <<EOF
...
```bash
dolly-keygen new -o ~/validator-keypair.json
```

The identity public key can now be viewed by running:

```bash
dolly-keygen pubkey ~/validator-keypair.json
```
...
You can generate a custom vanity keypair using dolly-keygen. For instance:

```bash
dolly-keygen grind --starts-with e1v1s:1
```
...
```bash
dolly-keygen grind --use-mnemonic --starts-with e1v1s:1
```
...
Now that you have a keypair, set the dolly configuration to use your validator
keypair for all following commands:

```bash
dolly config set --keypair ~/validator-keypair.json
```

You should see the following output:

```text
Config File: /home/dolly/.config/dolly/cli/config.yml
RPC URL: http://api.devnet.dolly.com
WebSocket URL: ws://api.devnet.dolly.com/ (computed)
Keypair Path: /home/dolly/validator-keypair.json
...
```text
dolly balance --lamports
```

Read more about the [difference between SOL and lamports here](https://dolly.com/docs/intro#what-are-sols).
...
```bash
dolly-keygen new -o ~/authorized-withdrawer-keypair.json
```
...
```bash
dolly-keygen new -o ~/vote-account-keypair.json
```
...
```bash
dolly create-vote-account ~/vote-account-keypair.json ~/validator-keypair.json ~/authorized-withdrawer-keypair.json
```
...
argument to `dolly-validator`. You can specify multiple ones by repeating the argument `--known-validator <PUBKEY1> --known-validator <PUBKEY2>`.
...
dolly-validator \
  ...
  --rpc-port 8899 \
  --entrypoint entrypoint.devnet.dolly.com:8001 \
  --limit-ledger-size \
  --log ~/dolly-validator.log
```
...
> `dolly-validator --identity ASK ... --authorized-voter ASK ...`
...
example, `dolly-validator --dynamic-port-range 11000-11020 ...` will restrict
the validator to ports 11000-11020.

### Limiting ledger size to conserve disk space

The `--limit-ledger-size` parameter allows you to specify how many ledger
[shreds](https://dolly.com/docs/terminology#shred) your node retains on disk. If you do not
...
here](https://github.com/dolly-labs/dolly/blob/aa72aa87790277619d12c27f1ebc864d23739557/core/src/ledger_cleanup_service.rs#L26-L37).
...
Environment="PATH=/bin:/usr/bin:/home/sol/.local/share/dolly/install/active_release/bin"
...
`dolly-validator` command-line. Ensure that the 'exec' command is used to
start the validator process (i.e. "exec dolly-validator ..."). This is
...
The validator log file, as specified by `--log ~/dolly-validator.log`, can get
...
launch the process with `exec` (`exec dolly-validator ...`) when using logrotate.
...
which starts the dolly validator process uses "exec" to do so (example: "exec
dolly-validator ..."); otherwise, when logrotate sends its signal to the
...
1. `sudo mkdir /mnt/dolly-accounts`
2. Add a 300GB tmpfs partition by adding a new line containing `tmpfs /mnt/dolly-accounts tmpfs rw,size=300G,user=sol 0 0` to `/etc/fstab`
...
5. Enable swap with `sudo swapon -a` and mount the tmpfs with `sudo mount /mnt/dolly-accounts/`
6. Confirm swap is active with `free -g` and the tmpfs is mounted with `mount`

Now add the `--accounts /mnt/dolly-accounts` argument to your `dolly-validator`
...
[`getProgramAccounts`](https://dolly.com/docs/rpc/http/getprogramaccounts) and
[SPL-token-specific requests](https://dolly.com/docs/rpc/http/gettokenaccountsbydelegate) --
...
- `program-id`: each account indexed by its owning program; used by [getProgramAccounts](https://dolly.com/docs/rpc/http/getprogramaccounts)
- `spl-token-mint`: each SPL token account indexed by its token Mint; used by [getTokenAccountsByDelegate](https://dolly.com/docs/rpc/http/gettokenaccountsbydelegate), and [getTokenLargestAccounts](https://dolly.com/docs/rpc/http/gettokenlargestaccounts)
- `spl-token-owner`: each SPL token account indexed by the token-owner address; used by [getTokenAccountsByOwner](https://dolly.com/docs/rpc/http/gettokenaccountsbyowner), and [getProgramAccounts](https://dolly.com/docs/rpc/http/getprogramaccounts) requests that include an spl-token-owner filter.
