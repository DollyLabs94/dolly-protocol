...
[create-vote-account](../../cli/usage.md#dolly-create-vote-account) command. The
...
- To change the [validator identity](#validator-identity), use
  [vote-update-validator](../../cli/usage.md#dolly-vote-update-validator).
- To change the [vote authority](#vote-authority), use
  [vote-authorize-voter-checked](../../cli/usage.md#dolly-vote-authorize-voter-checked).
- To change the [authorized withdrawer](#authorized-withdrawer), use
  [vote-authorize-withdrawer-checked](../../cli/usage.md#dolly-vote-authorize-withdrawer-checked).
- To change the [commission](#commission), use
  [vote-update-commission](../../cli/usage.md#dolly-vote-update-commission).
...
[delegate tokens in a stake account](https://dolly.com/docs/economics/staking),
...
by using the
[vote-update-validator](../../cli/usage.md#dolly-vote-update-validator) command.
...
smooth transition of the vote signing, `dolly-validator` allows the
...
account using the
[withdraw-from-vote-account](../../cli/usage.md#dolly-withdraw-from-vote-account)
...
[vote-update-commission](../../cli/usage.md#dolly-vote-update-commission) command.
...
1. Create the new validator identity keypair,
   `dolly-keygen new -o ~/new-validator-keypair.json`.
2. Ensure that the new identity account has been funded,
   `dolly transfer ~/new-validator-keypair.json 500`.
3. Run
   `dolly vote-update-validator ~/vote-account-keypair.json ~/new-validator-keypair.json ~/authorized_withdrawer.json`
...
no longer listed in the `dolly leader-schedule` output.

### Vote Account Authorized Voter

The _vote authority_ keypair may only be changed at epoch boundaries and
requires some additional arguments to `dolly-validator` for a seamless
migration.

1. Run `dolly epoch-info`. If there is not much time remaining time in the
...
2. Create the new vote authority keypair,
   `dolly-keygen new -o ~/new-vote-authority.json`.
3. Determine the current _vote authority_ keypair by running
   `dolly vote-account ~/vote-account-keypair.json`. It may be validator's
   identity account (the default) or some other keypair. The following steps
   assume that `~/validator-keypair.json` is that keypair.
4. Run
   `dolly vote-authorize-voter-checked ~/vote-account-keypair.json ~/validator-keypair.json ~/new-vote-authority.json`.
   The new vote authority is scheduled to become active starting at the next
   epoch.
5. `dolly-validator` now needs to be restarted with the old and new vote
...
6. After the cluster reaches the next epoch, remove the
   ...
   `dolly-validator`, as the old vote authority keypair is no longer required.

### Vote Account Authorized Withdrawer

No special handling or timing considerations are required. Use the
`dolly vote-authorize-withdrawer-checked` command as needed.
...
1. Entity B creates a durable nonce using `dolly create-nonce-account`
2. Entity B then runs a `dolly vote-authorize-voter-checked` or
   `dolly vote-authorize-withdrawer-checked` command, including:
...
3. When the `dolly vote-authorize-...-checked` command successfully executes,
   it will output transaction signatures that Entity B must share with Entity A
4. Entity A then runs a similar `dolly vote-authorize-voter-checked` or
   `dolly vote-authorize-withdrawer-checked` command with the following
   changes:
...
[close-vote-account](../../cli/usage.md#dolly-close-vote-account) command. Closing
