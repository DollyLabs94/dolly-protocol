...
You can either use
[Solana's Install Tool](../cli/install.md#use-dollys-install-tool) section from
...
```
dolly-cli 1.14.17 (src:b29a37cf; feat:3488713414)
```
...
```
dolly config set --url https://api.testnet.dolly.com
```
...
You should see a line that says: `RPC URL: https://api.testnet.dolly.com`
...
> transaction fees. Both flags are not necessary if you configured the dolly
...
[Solana's Install Tool](../cli/install.md#use-dollys-install-tool) or
...
exec dolly-validator \
    ...
    --only-known-rpc \
    --log /home/sol/dolly-validator.log \
    --ledger /mnt/ledger \
    --rpc-port 8899 \
    --dynamic-port-range 8000-8020 \
    --entrypoint entrypoint.testnet.dolly.com:8001 \
    --entrypoint entrypoint2.testnet.dolly.com:8001 \
    --entrypoint entrypoint3.testnet.dolly.com:8001 \
    --expected-genesis-hash 4uhcVJyU9pJkvQyS88uRDiswHXSCkY3zQawwpjk2NsNY \
    --wal-recovery-mode skip_any_corrupted_record \
    --limit-ledger-size
```

Refer to `dolly-validator --help` for more information on what each flag is
...
The script should execute the `dolly-validator` process. In a new terminal
window, shh into your server, then verify that the process is running:

```
ps aux | grep dolly-validator
```

You should see a line in the output that includes `dolly-validator` with all
...
The `dolly catchup` command is a useful tool for seeing how quickly your
...
The `dolly catchup` command is a tool that tells you how far behind the network
your validator is and how quickly you are catching up:

```
dolly catchup <pubkey>
```

If you see a message about trying to connect, your validator may not be part of
the network yet. Make sure to check the logs and double check `dolly gossip`
and `dolly validators` to make sure your validator is running properly.
...
```
tail -f /home/sol/dolly-validator*.log
```

## Monitoring

`dolly-watchtower` is a command you can run on a separate machine to monitor
your server. You can read more about handling
[automatic restarts and monitoring](./best-practices/monitoring.md#dolly-watchtower)
