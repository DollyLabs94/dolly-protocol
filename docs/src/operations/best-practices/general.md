...
From within the Solana CLI, you can execute the `dolly-validator` command with
the `--help` flag to get a better understanding of the flags and sub commands
available.

```
dolly-validator --help
```
...
[leader slot](https://dolly.com/docs/terminology#leader-schedule) is the time
...
Assuming you are ready to restart, you may use the `dolly-validator exit`
...
your `dolly-validator` binary to your specific hardware.
...
`dolly-install` command.

Assuming you want to install Solana version `1.14.17`, you would execute the
following:

```
dolly-install init 1.14.17
```

This command downloads the executable for `1.14.17` and installs it into a
`.local` directory. You can also look at `dolly-install --help` for more
options.

> **Note** this command only works if you already have the dolly cli installed.
> If you do not have the cli installed, refer to
> [install dolly cli tools](../../cli/install.md)

### Restart

For all install methods, the validator process will need to be restarted before
the newly installed version is in use. Use `dolly-validator exit` to restart
...
`dolly-validator` command:

```
--no-snapshot-fetch
```

If you use this flag with the `dolly-validator` command, make sure that you run
`dolly catchup <pubkey>` after your validator starts to make sure that the
...
address of the validator in using the `dolly gossip` command. In the example
below, `5D1fNXzvv5NjV1ysLjirC4WY92RNsVH18vjmcszZd8on` is the pubkey of one of my
known validators:

```
dolly gossip | grep 5D1fNXzvv5NjV1ysLjirC4WY92RNsVH18vjmcszZd8on
```
...
running the `dolly-validator` software. How much SOL you should store there is
...
```
dolly balance validator-keypair.json
```

> **Note** `dolly-watchtower` can monitor for a minimum validator identity
...
`dolly withdraw-from-vote-account` on a trusted computer. For example, on a
...
```
dolly withdraw-from-vote-account \
...
`dolly withdraw-from-vote-account --help`.
