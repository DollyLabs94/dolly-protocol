...
echo 'Building the dolly cli from source...'
...
cargo build -p dolly-cli

usage=$(cargo -q run -p dolly-cli -- -C ~/.foo --help | sed -e 's|'"$HOME"'|~|g' -e 's/[[:space:]]\+$//')
...
while read -r subcommand rest; do
  ...
  if ((in_subcommands)); then
      section "$(cargo -q run -p dolly-cli -- help "$subcommand" | sed -e 's|'"$HOME"'|~|g' -e 's/[[:space:]]\+$//')" "###" >> "$out"
  fi
done <<<"$usage">>"$out"
