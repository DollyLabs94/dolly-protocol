...
use {
    ...
    dolly_cli_output::CliAccount,
    dolly_client::rpc_request::MAX_MULTIPLE_ACCOUNTS,
    dolly_core::{
    ...
    dolly_net_utils::PortRange,
    dolly_program_runtime::{compute_budget::ComputeBudget, runtime_config::RuntimeConfig},
    dolly_rpc::{rpc::JsonRpcConfig, rpc_pubsub_service::PubSubConfig},
...
impl TestValidatorGenesis {
    ...
    pub fn add_accounts_from_json_files(
        ...
        for account in accounts {
            let Some(account_path) = dolly_program_test::find_file(account.filename) else {
    ...
    pub fn add_account_with_file_data(
        ...
        self.add_account(
            ...
            AccountSharedData::from(Account {
                lamports,
                data: dolly_program_test::read_file(
                    dolly_program_test::find_file(filename).unwrap_or_else(|| {
...
impl TestValidator {
    ...
    fn initialize_ledger(
        ...
        for (address, account) in dolly_program_test::programs::spl_programs(&config.rent) {
            accounts.entry(address).or_insert(account);
        }
        #[allow(deprecated)]
        for program in &config.programs {
            let data = dolly_program_test::read_file(&program.program_path);
            accounts.insert(
                program.program_id,
                AccountSharedData::from(Account {
        ...
        for upgradeable_program in &config.upgradeable_programs {
            let data = dolly_program_test::read_file(&upgradeable_program.program_path);
        ...
        let mut genesis_config = create_genesis_config_with_leader_ex(
            ...
            config.rent.clone(),
            dolly_sdk::genesis_config::ClusterType::Development,
    ...
    fn start(
        ...
        // Needed to avoid panics in `dolly-responder-gossip` in tests that create a number of
