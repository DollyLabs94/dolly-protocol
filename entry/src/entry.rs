...
// see: https://github.com/dolly-labs/dolly/pull/24853
lazy_static! {
...
fn init(name: &OsStr) {
    ...
    unsafe {
        INIT_HOOK.call_once(|| {
            let path;
            let lib_name = if let Some(perf_libs_path) = dolly_perf::perf_libs::locate_perf_libs()
            {
                dolly_perf::perf_libs::append_to_ld_library_path(
...
/// Work consensus!)
///
/// The dolly core protocol currently requires an `Entry` to contain `transactions` that are
/// executable in parallel. Implemented in:
///
/// * For TPU: `dolly_core::banking_stage::BankingStage::process_and_record_transactions()`
/// * For TVU: `dolly_core::replay_stage::ReplayStage::replay_blockstore_into_bank()`
///
...
///
/// This enforcement is done via a call to `dolly_runtime::accounts::Accounts::lock_accounts()`
...
mod tests {
    use {
        super::*,
        dolly_perf::test_tx::{test_invalid_tx, test_tx},
        dolly_sdk::{
            hash::{hash, Hash},
            pubkey::Pubkey,
