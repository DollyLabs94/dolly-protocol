...
FLAGS:
        ...
        --no-dolly-prefix    Do not require \`dolly-\` prefix on PACKAGE_NAME
...
if ${require_dolly_prefix} && ! [[ "${package_name}" =~ ^dolly- ]]; then
  # shellcheck disable=SC2016 # backticks are not a command here
  echo 'error: PACKAGE_NAME MUST start with `dolly-`' 1>&2
  display_help
  exit 1
fi
