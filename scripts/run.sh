...
ok=true
for program in dolly-{faucet,genesis,keygen,validator}; do
  $program -V || ok=false
done
$ok || {
  echo
...
export RUST_LOG=${RUST_LOG:-dolly=info,dolly_runtime::message_processor=debug} # if RUST_LOG is unset, default to info
...
if [[ -e $validator_vote_account ]]; then
  ...
  dolly-keygen new --no-passphrase -so "$validator_vote_account"
fi
...
if [[ -e $validator_stake_account ]]; then
  ...
  dolly-keygen new --no-passphrase -so "$validator_stake_account"
fi
...
# shellcheck disable=SC2086
dolly-validator "${args[@]}" $SOLANA_RUN_SH_VALIDATOR_ARGS &
validator=$!
