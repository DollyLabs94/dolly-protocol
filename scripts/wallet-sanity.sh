#!/usr/bin/env bash
#
# dolly-cli integration sanity test
#
set -e
...
while [[ $timeout -gt 0 ]]; do
  set +e
  output=$($dolly_cli "${args[@]}" transaction-count --commitment finalized)
  rc=$?
  set -e
...
(
  set -x
  $dolly_cli "${args[@]}" address
  $dolly_cli "${args[@]}" balance
  $dolly_cli "${args[@]}" ping --count 5 --interval 0
  $dolly_cli "${args[@]}" balance
)
