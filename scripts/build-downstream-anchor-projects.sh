#!/usr/bin/env bash
#
# Builds known downstream projects against local dolly source
#
...
anchor_version=$1
dolly_ver=$(readCargoVariable version Cargo.toml)
dolly_dir=$PWD
cargo="$dolly_dir"/cargo
cargo_build_sbf="$dolly_dir"/cargo-build-sbf
cargo_test_sbf="$dolly_dir"/cargo-test-sbf
...
anchor() {
  ...
  cp "$dolly_dir"/rust-toolchain.toml .

  update_dolly_dependencies . "$dolly_ver"
  patch_crates_io_dolly Cargo.toml "$dolly_dir"
  patch_spl_crates . Cargo.toml "$spl_dir"

  $cargo test
  # serum_dex and mpl-token-metadata are using caret versions of dolly and SPL dependencies
  ...
  cd "$dolly_dir"/target/downstream-projects-anchor
}
...
mango() {
  (
    ...
    cd mango-v4
    update_dolly_dependencies . "$dolly_ver"
    patch_crates_io_dolly_no_header Cargo.toml "$dolly_dir"
    $cargo_test_sbf --features enable-gpl
  )
}
...
metaplex() {
  (
    ...
    # copy toolchain file to use dolly's rust version
    cp "$dolly_dir"/rust-toolchain.toml mpl-token-metadata/
