...
if [[ -n $SOLANA_CUDA ]]; then
  program=$dolly_validator_cuda
else
  program=$dolly_validator
fi
...
wallet() {
  (
    set -x
    $dolly_cli --keypair "$identity" --url "$rpc_url" "$@"
  )
}
...
rpc_url=$($dolly_gossip --allow-private-addr rpc-url --timeout 180 --entrypoint "$gossip_entrypoint")

[[ -r "$identity" ]] || $dolly_keygen new --no-passphrase -so "$identity"
[[ -r "$vote_account" ]] || $dolly_keygen new --no-passphrase -so "$vote_account"
[[ -r "$authorized_withdrawer" ]] || $dolly_keygen new --no-passphrase -so "$authorized_withdrawer"
