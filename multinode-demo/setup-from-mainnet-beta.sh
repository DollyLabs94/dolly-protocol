...
(
  ...
  set -x
  wget http://api.mainnet-beta.dolly.com/genesis.tar.bz2
  wget --trust-server-names http://api.mainnet-beta.dolly.com/snapshot.tar.bz2
)
...
if [[ -r $FAUCET_KEYPAIR ]]; then
  ...
  $dolly_keygen new --no-passphrase -fso "$SOLANA_CONFIG_DIR"/faucet.json
fi

if [[ -f $BOOTSTRAP_VALIDATOR_IDENTITY_KEYPAIR ]]; then
  cp -f "$BOOTSTRAP_VALIDATOR_IDENTITY_KEYPAIR" "$SOLANA_CONFIG_DIR"/bootstrap-validator/identity.json
else
  $dolly_keygen new --no-passphrase -so "$SOLANA_CONFIG_DIR"/bootstrap-validator/identity.json
fi

$dolly_keygen new --no-passphrase -so "$SOLANA_CONFIG_DIR"/bootstrap-validator/vote-account.json
$dolly_keygen new --no-passphrase -so "$SOLANA_CONFIG_DIR"/bootstrap-validator/stake-account.json

$dolly_ledger_tool create-snapshot \
...
$dolly_ledger_tool modify-genesis \
  --ledger "$SOLANA_CONFIG_DIR"/latest-mainnet-beta-snapshot \
  --hashes-per-tick sleep \
