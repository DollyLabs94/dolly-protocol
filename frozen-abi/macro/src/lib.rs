...
fn derive_abi_sample_enum_type(input: ItemEnum) -> TokenStream {
    ...
    let result = quote! {
        ...
        #( #attrs )*
        impl #impl_generics ::dolly_frozen_abi::abi_example::AbiExample for #type_name #ty_generics #where_clause {
            fn example() -> Self {
                ::log::info!(
                    "AbiExample for enum: {}",
...
fn do_derive_abi_enum_visitor(input: ItemEnum) -> TokenStream {
    ...
    (quote! {
        impl #impl_generics ::dolly_frozen_abi::abi_example::AbiEnumVisitor for #type_name #ty_generics #where_clause {
            fn visit_for_abi(&self, digester: &mut ::dolly_frozen_abi::abi_digester::AbiDigester) -> ::dolly_frozen_abi::abi_digester::DigestResult {
...
fn quote_for_test(
    ...
    quote! {
        ...
        mod #test_mod_ident {
            use super::*;
            use ::dolly_frozen_abi::abi_example::{AbiExample, AbiEnumVisitor};

            #[test]
            fn test_abi_digest() {
                ::dolly_logger::setup();
                let mut digester = ::dolly_frozen_abi::abi_digester::AbiDigester::create();
