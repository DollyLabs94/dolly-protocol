use {
    ...
    log::*,
    dolly_account_decoder::{UiAccountEncoding, UiDataSliceConfig},
    ...
    dolly_program_runtime::{compute_budget::ComputeBudget, invoke_context::InvokeContext},
    dolly_rbpf::{elf::Executable, verifier::RequisiteVerifier},
...
impl ProgramSubCommands for App<'_, '_> {
    fn program_subcommands(self) -> Self {
        ...
        .subcommand(
            SubCommand::with_name("deploy")
                .about(
                    "Deploy has been removed. Use `dolly program deploy` instead to deploy \
        ...
        if let Some(write_signer) = write_signer {
            ...
            let transaction_errors = match connection_cache {
                ...
                ConnectionCache::Quic(cache) => {
                    let tpu_client_fut = dolly_client::nonblocking::tpu_client::TpuClient::new_with_connection_cache(
                        rpc_client.get_inner_client().clone(),
                        config.websocket_url.as_str(),
                        dolly_client::tpu_client::TpuClientConfig::default(),
                        cache,
                    );
...
fn report_ephemeral_mnemonic(words: usize, mnemonic: bip39::Mnemonic) {
    ...
    eprintln!("`dolly-keygen recover` and the following {words}-word seed phrase:");
    eprintln!("{divider}\n{phrase}\n{divider}");
    eprintln!("To resume a deploy, pass the recovered keypair as the");
    eprintln!("[BUFFER_SIGNER] to `dolly program deploy` or `dolly program write-buffer'.");
    eprintln!("Or to recover the account's lamports, pass it as the");
    eprintln!("[BUFFER_ACCOUNT_ADDRESS] argument to `dolly program close`.\n{divider}");
}
...
mod tests {
    use {
        ...
        serde_json::Value,
        dolly_cli_output::OutputFormat,
        dolly_sdk::{hash::Hash, signature::write_keypair_file},
    };
