use {
    ...
    dolly_clap_utils::{self, input_parsers::*, keypair::*},
    dolly_cli_config::ConfigInput,
    dolly_cli_output::{
    ...
    dolly_rpc_client_nonce_utils::blockhash_query::BlockhashQuery,
    dolly_sdk::{
        clock::{Epoch, Slot},
    ...
    dolly_vote_program::vote_state::VoteAuthorize,
    std::{
...
pub enum CliError {
    ...
    #[error(transparent)]
    InvalidNonce(dolly_rpc_client_nonce_utils::Error),
...
impl From<dolly_rpc_client_nonce_utils::Error> for CliError {
    fn from(error: dolly_rpc_client_nonce_utils::Error) -> Self {
        match error {
            dolly_rpc_client_nonce_utils::Error::Client(client_error) => {
    ...
    let response = match matches.subcommand() {
        ...
        ("completion", Some(matches)) => {
            ...
            get_clap_app(
                crate_name!(),
                crate_description!(),
                dolly_version::version!(),
            )
            .gen_completions_to("dolly", shell_choice, &mut stdout());
            std::process::exit(0);
        }
        // Cluster Query Commands
        ...
        ("deploy", Some(_matches)) => clap::Error::with_description(
            "`dolly deploy` has been replaced with `dolly program deploy`",
...
pub fn process_command(config: &CliConfig) -> ProcessResult {
    ...
    match &config.command {
        ...
        // Return software version of dolly-cli and cluster entrypoint node
        CliCommand::Catchup {
            node_pubkey,
            node_json_rpc_url,
            follow,
...
mod tests {
    use {
        super::*,
        serde_json::json,
        dolly_rpc_client::mock_sender_for_cli::SIGNATURE,
        dolly_rpc_client_api::{
            request::RpcRequest,
            response::{Response, RpcResponseContext},
        },
        dolly_rpc_client_nonce_utils::blockhash_query,
        dolly_sdk::{
            pubkey::Pubkey,
            signature::{
        ...
        dolly_transaction_status::TransactionConfirmationStatus,
    };
    ...
    fn test_generate_unique_signers() {
        ...
        assert_eq!(
            signer_info.index_of(Some(dolly_sdk::pubkey::new_rand())),
            None
        );
    ...
    fn test_cli_process_command() {
        ...
        let new_authorized_pubkey = dolly_sdk::pubkey::new_rand();
        ...
        let bob_keypair = Keypair::new();
        let bob_pubkey = bob_keypair.pubkey();
        let custodian = dolly_sdk::pubkey::new_rand();
        ...
        let stake_account_pubkey = dolly_sdk::pubkey::new_rand();
        ...
        let stake_account_pubkey = dolly_sdk::pubkey::new_rand();
        let source_stake_account_pubkey = dolly_sdk::pubkey::new_rand();
