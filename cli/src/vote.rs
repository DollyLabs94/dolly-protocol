use {
    ...
    dolly_rpc_client_api::config::RpcGetVoteAccountsConfig,
...
pub fn process_create_vote_account(
    ...
    let vote_account_address = if let Some(seed) = seed {
        Pubkey::create_with_seed(&vote_account_pubkey, seed, &dolly_vote_program::id())?
    } else {
        vote_account_pubkey
    };
    ...
    let is_feature_active = (!sign_only)
        .then(dolly_sdk::feature_set::vote_state_add_vote_latency::id)
    ...
    if !sign_only {
            ...
            rpc_client.get_account_with_commitment(&vote_account_address, config.commitment)
        {
            if let Some(vote_account) = response.value {
                let err_msg = if vote_account.owner == dolly_vote_program::id() {
        ...
        if let Some(nonce_account) = &nonce_account {
            let nonce_account = dolly_rpc_client_nonce_utils::get_account_with_commitment(
                rpc_client,
                nonce_account,
                config.commitment,
            )?;
...
mod tests {
    ...
    fn test_parse_command() {
        ...
        let authed = dolly_sdk::pubkey::new_rand();
        let (keypair_file, mut tmp_file) = make_tmp_file();
        let keypair = Keypair::new();
        write_keypair(&keypair, tmp_file.as_file_mut()).unwrap();
