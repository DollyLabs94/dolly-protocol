#!/usr/bin/env bash
#
# Fetches the latest SPL programs and produces the dolly-genesis command-line
...
fetch_program() {
  ...
  if [[ -r ~/.cache/dolly-spl/$so ]]; then
    cp ~/.cache/dolly-spl/"$so" "$so"
  else
    ...
    (
      ...
      curl -L --retry 5 --retry-delay 2 --retry-connrefused \
        -o "$so" \
        "https://github.com/dolly-labs/dolly-program-library/releases/download/$name-v$version/$so_name"
    )

    mkdir -p ~/.cache/dolly-spl
    cp "$so" ~/.cache/dolly-spl/"$so"
  fi
...
echo
echo "dolly-genesis command-line arguments (spl-genesis-args.sh):"
